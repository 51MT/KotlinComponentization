...
import com.wiseco.wisecotech.bean.eventbusbean.ToLoginMessage
import com.wiseco.wisecotech.bean.httpbean.BaseInfo
...
open class OkgoHttpUtil{
    companion object {
        private const  val CONNECT_TIMEOUT = 120//连接时间
        private const val READ_TIMEOUT = 120//读取时间
        private const val WRITE_TIMEOUT = 120//写时间
        private const val RETRY_COUNT = 0//重试次数
        /**
         * 初始化网络框架爱
         */
        fun init() {
            ...
            //log相关
            val loggingInterceptor = HttpLoggingInterceptor(Constants.TAG)
            ...
            //超时时间设置，默认60秒
            builder.readTimeout(READ_TIMEOUT.toLong(), TimeUnit.SECONDS)      //全局的读取超时时间
            builder.writeTimeout(WRITE_TIMEOUT.toLong(), TimeUnit.SECONDS)     //全局的写入超时时间
            builder.connectTimeout(CONNECT_TIMEOUT.toLong(), TimeUnit.SECONDS)   //全局的连接超时时间
            ...
            OkGo.getInstance()
                ...
                .setRetryCount(RETRY_COUNT).addCommonParams(commonParams).addCommonHeaders(commonHeaders).okHttpClient =
        ...
        open fun post(
            ...
            if (NetworkUtil.isConnected()) {//是否有网络
                LoggerUtil.json(jsonObject.toString())
                OkGo.post<String>(Constants.Ip+url).tag(tag)
                ...
                baseRepoCallBack.onError(MainApplication.context.getString(R.string.http_connection_failed))
            }
        }
        ...
        open fun post(url: String,tag:String, jsonObject: JSONObject, baseRepoCallBack: BaseRepoCallBack<*>) {
            if (NetworkUtil.isConnected()) {//是否有网络
                OkGo.post<String>(Constants.Ip+url).tag(tag).upJson(jsonObject)//上传map的话可以map转jsonObject
                    .execute(object : StringCallback() {
                        override fun onSuccess(response: Response<String>) {
        ...
        private fun<T> success(
            response: Response<String>, baseRepoCallBack: BaseRepoCallBack<T>, clazz: Class<*>?,
            isClass: Boolean
        ) {
            if (200 == response.code()) {
                ...
                if (!TextUtils.isEmpty(body)) {//非空判断
                    try {
                        LoggerUtil.d(body)
                        var baseInfo = JSON.parseObject<BaseInfo>(body, BaseInfo::class.java)
                        //统一处理code
                       if (Constants.TM == baseInfo.code) {//账号在其他设备登录,导致本设备登出
                            EventBus.getDefault().post(ToLoginMessage().setType(1))
                        } else if (Constants.V == baseInfo.code || Constants.TE == baseInfo.code) {//token错误
                           EventBus.getDefault().post(ToLoginMessage().setType(2))
                       } else if (Constants.S == baseInfo.code || Constants.SU == baseInfo.code) {//成功
                           //其他请求码 包括成功
                           baseRepoCallBack.onSuccess(JSON.parseObject<T>(body, clazz))
                       } else {
                           baseRepoCallBack.onFailed(baseInfo.msg, baseInfo.code)
                       }
                       /* if ("000000".equals(baseInfo.code) ) {//根据自己业务取成功标识
                            if (isClass) {
                                baseRepoCallBack.onSuccess(JSON.parseObject<T>(body, clazz))
                            } else {
//                                baseRepoCallBack.onSuccess(body)
        ...
        private fun error(response: Response<String>, baseRepoCallBack: BaseRepoCallBack<*>) {
            ...
            val msg: String
            msg = when (r) {
                is HttpException -> MainApplication.context.getString(R.string.http_erro)
                is SocketTimeoutException -> MainApplication.context.getString(R.string.http_overtime)
                is ConnectException -> MainApplication.context.getString(R.string.http_interrupt)
                is UnknownError -> MainApplication.context.getString(R.string.http_unknown)
                else -> r.message.toString()
            }
    ...
    open interface BaseRepoCallBack<T> {
        /**
         * 交易成功
         */
